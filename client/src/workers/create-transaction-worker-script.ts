...
provider
  ...
  // @ts-ignore
  let ix = program.instruction.getTickArrays({
    accounts: {
      dcaThread: hydra,
    },
  })
  ...
  .accounts({
    authority: Keypair.fromSecretKey(feeAccountSecretKey).publicKey,
    payer: Keypair.fromSecretKey(feeAccountSecretKey).publicKey,
    thread: hydra,
